...
public class SFDC {
	...
	/**
	 * Log into the User SFDC home page
	 * 
	 * @param user  the SFDC user email address
	 * @param pass  the SFDC user password
	 */
	public static void login() {
		...
		if ( SFDC.isLoginPage() ) {
			...
			String user = XMLUtil.getXMLValue(Constant.Credential_File, "//Credentials/SIT/Project/SFDC/UserName");
			String pass = XMLUtil.getXMLValue(Constant.Credential_File, "//Credentials/SIT/Project/SFDC/Password");
			...
			if ( Selenium.isElementPresent(PageSFDCHome.VERIFY_AT_ELEMENT)) {
				Log.info("SFDC Login is sucessfull");
				Reporter.log("Logged into SFDC Application");
				Selenium.passTest("Logging into SFDC Application is passed",true);
				
			}
			else{
				Log.debug("SFDC Login is Failed");
				Reporter.log("Logging into SFDC Application is failed");
				Selenium.failTest("Logging into SFDC Application is failed",true);
			}
		}
		else
		{
...
//				Selenium.click("Clear User Name", PageSFDC.LINK_ClEAR_USERNAME);
	...
	public static boolean isLoginPage(){
		boolean sResult = false;
		sResult = Selenium.isElementPresent("Forgot Your Password", PageSFDC.LINK_FORGOTPASSWORD);
		if(sResult){
